$(document).ready(function () {
    ...
    const default_opening = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1';

    var game_started = false;
    var game_opening = default_opening;
    var game_mode = 'ovc';
    var time = 0;
    var black_time = 0;
    var white_time = 0;
    ...
    function checkStatus(color) {
        if (game.in_checkmate()) {
            $('#status').html(`Oops Checkmate! , <b>${(color == 'white') ? 'Black' : 'White'}</b> Won.`);
            ...
            $('#status').html(`${color}'s turn to move`);
            return false;
        }
    ...
    function updateAdvantage() {
        if (globalSum > 0) {
            ...
            $('#advantageBar').removeClass('bg-success');
            $('#advantageBar').addClass('bg-danger');
        }
        else if (globalSum < 0) {
            $('#advantageColor').text('White');
            $('#advantageNumber').text(-globalSum);
            $('#advantageBar').removeClass('bg-danger');
            $('#advantageBar').addClass('bg-success');
        }
        else {
    ...
    function makeBestMove(color) {
        ...
        //update moves list
        upadteMovesList(move);

        // update time
        updateTime(move.color);
    ...
    function reset() {
        ...
        $('#white-total-time').text("00:00");
        $('#white-lastmove-time').text("00:00");

        $('#black-total-time').text("00:00");
        $('#black-lastmove-time').text("00:00");
        ...
        // enable startBtn
        game_started = false;
        $('#startBtn').removeClass('disableBtn');
    ...
    $('#ruyLopezBtn').on('click', function () {
        game_opening = 'r1bqkbnr/pppp1ppp/2n5/1B2p3/4P3/5N2/PPPP1PPP/RNBQK2R b KQkq - 0 1';
        $('#game-opening').text('Ruy Lopez');
    });
    $('#italianGameBtn').on('click', function () {
        game_opening = 'r1bqkbnr/pppp1ppp/2n5/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R b KQkq - 0 1'
        $('#game-opening').text('Italian Game');
    });
    $('#sicilianDefenseBtn').on('click', function () {
        game_opening = 'rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1';
        $('#game-opening').text('Sicilian Defense');
    });
    $('#startingPositionBtn').on('click', function () {
        game_opening = default_opening;
        $('#game-opening').text('Starting Position');
    });

    $('#oneVsOneBtn').on('click', function () {
        game_mode = 'ovo';
        $('#game-mode').text('One -vs- One');
    })
    $('#oneVsCompBtn').on('click', function () {
        game_mode = 'ovc';
        $('#game-mode').text('One -vs- Comp');
    });
    $('#compVsCompBtn').on('click', function () {
        game_mode = 'cvc';
        $('#game-mode').text('Comp -vs- Comp');
    });
    ...
    $('#startBtn').on('click', function (e) {
        ...
        reset();
        game_started = true;
        $('#startBtn').addClass('disableBtn');

        game.load(game_opening);
        board.position(game.fen());
        ...
        countdownInterval = setInterval(function () {
            ...
            $('#countdown-time').text(formatTime(diff));
        }, 100);


        // some opening positions need to be played by black first.
        // so, move black if game mode is ovc.
        if (game_mode === 'ovc' && game.turn() === 'b') {
            window.setTimeout(function () {
                makeBestMove('b');
            }, 250);
        }

        // if game mode is cvc, start playing computer vs. computer.
        if (game_mode === 'cvc') startCompVsComp();
    ...
    function onDrop(source, target) {
        ...
        // if game not started, snapback pieces
        if (!game_started) return 'snapback';
        ...
        // game_mode == 'ovo'
        if (game_mode == 'ovo') {
            game.turn() === 'w' ? checkStatus('white') : checkStatus('black');
        }

        // make move for black if game_mode = 'ovc'
        if (game_mode == 'ovc') {
            // if game not over
    ...
    function upadteMovesList(move) {
        // console.log(move);

        if (move.color === 'w') {
            $('#white-moves').append('<li>' + move.san + '</li>');
        }
        else {
            $('#black-moves').append('<li>' + move.san + '</li>');
        }
    ...
    function updateTime(color) {
        var diff = new Date() - time;

        if (color === 'w') {
            white_time += diff;
            $('#white-total-time').text(formatTime(white_time));
            $('#white-lastmove-time').text(formatTime(diff));
        }
        else if (color === 'b') {
            black_time += diff;
            $('#black-total-time').text(formatTime(black_time));
            $('#black-lastmove-time').text(formatTime(diff));
        }
    ...
    // resize the board size based on window size
